...
export default function EditQuestionForm({ isOpen, onClose, onSuccess, question, organization, tasks = [] }) {
    const { data, setData, put, processing, errors, reset } = useForm({
        ...
        questionable_type: question?.questionable_type || 'App\\Models\\LessonTask',
        questionable_id: question?.questionable_id || '',
        options: (question?.options || []).map((opt, idx) => ({
            text: opt.text,
            is_correct: question?.correct_answers?.includes(String(idx)) || false,
        })),
        correct_answers: question?.correct_answers || [],
        text_answer: question?.question_type === 'text' ? (question?.correct_answers?.[0] ?? '') : '',
    });
    ...
    const submit = (e) => {
        ...
        put(route('question.update', { organization: organization.id, question: question.id}), {
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                reset();
