...
export default function EditExamForm({ isOpen, onClose, onSuccess, exam, organization, modules }) {
    const { data, setData, put, processing, errors, reset } = useForm({
        title: exam?.title || '',
        description: exam?.description || '',
        module_id: exam?.module_id || '',
        is_active: exam?.is_active ? true : false,
        time_limit: exam?.time_limit ? Number(exam.time_limit) : '',
    ...
    const submit = (e) => {
        e.preventDefault();
        put(
            route('exam.update', { organization: organization.id, exam: exam.id }),
            {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
                            ...
                            <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl glass-card p-6 text-left align-middle shadow-xl transition-all animate-fade-in">
                                ...
                                <form onSubmit={submit} className="space-y-5">
                                    ...
                                    <div>
                                        ...
                                        <Listbox value={data.module_id} onChange={(value) => setData('module_id', value)}>
                                            <div className="relative">
                                                <Listbox.Button className="form-input-glass w-full text-left flex items-center justify-between pr-10">
                                                    <span className={selectedModule ? 'text-main' : 'text-meta'}>
                                        ...
                                        {errors.module_id && <p className="mt-1 text-sm text-red-500">{errors.module_id}</p>}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Описание контрольной работы"
                                        />
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-meta">⏱️</span>
                                            <input
                                                type="number"
                                                value={data.time_limit}
                                                onChange={(e) => setData('time_limit', e.target.value)}
                                                className="form-input-glass w-full pl-10"
                                                placeholder="60"
                                            />
                                        </div>
                                        {errors.time_limit && <p className="mt-1 text-sm text-red-500">{errors.time_limit}</p>}
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between py-2">
                                        <span className="text-sm font-medium text-main">Сделать активной</span>
                                        <Switch
